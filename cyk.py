    ...
    for m in range(1, length):
        for n in range(0, length - m):
            ...
            for k in range(n+1, n+m+1):
                for nt in nt_rules:
                    if chart[k-n-1][n].get(nt.rhs1) and chart[n+m-k][k].get(nt.rhs2):
                        chart[m][n][nt.lhs] = ParsedPhrase(label=nt.lhs, children=[chart[k-n-1][n].get(nt.rhs1), chart[n+m-k][k].get(nt.rhs2)])
    # END TODO
